...
exports.teviewscores = function(req,res){
	...
	Score.find({exam_id:req.query.examId},function(err,doc){
		...
		// console.log(stuuserId,final_score);
...
exports.teahistroys = function(req,res){
	...
	Score.find({exam_id:examId},function(err,mis){
		...
		console.log('222222',arrstuhistory);
		res.json({
			errcode : 1,
			errmsg : '访问学生成绩详细成功',
...
exports.jsuandiff = function(req,res){
	// console.log('111122223333334444444');
	// 所有习题的id号
	var testid = [];
	TestDb.find(function(err,doc){
		...
		// console.log('44444444455555555555555555555');
		// 所有学生答题的详情
		var mistakesarr = [];
		Score.find(function(err,data){
			...
			// console.log('mistakesarr',mistakesarr);
			var titrue = [];
			var json = [];
			...
			for(var m=0;m<testid.length;m++){
				...
				for(var i=0;i<mistakesarr.length;i++){
					for(var j=0;j<mistakesarr[i].length;j++){
						// console.log('mistakesarr[i].testdb_id',mistakesarr[i][j].testdb_id);
						if(testid[m] == mistakesarr[i][j].testdb_id){
							/*if(testid[m] == '56eb8e3eec782e290301ec4f'){
								console.log(123454321);
							}*/
							// console.log('ffffffffffffffff',mistakesarr[i][j].testdb_id);
			...
			*/
			...
			for(var m=0;m<titrue.length;m++){
				...
				// console.log((parseInt(titrue[m].index)/parseInt(titrue[m].istrue.length)).toFixed(2));
				if(pointer > 0.6){
					titrue[m].diff = '很难'
				}else if(pointer > 0.3){
					titrue[m].diff = '适中'
